...
const hitterRows=hitterData.base.map((r,i)=>{const id=Number(r.player_id);return{playerId:id,rank:i+1,player:r.player_name,pitches:number(r.total_pitches),pa:number(r.pa),bbPct:number(r.bb_percent),totalStrikes:number(hitterMaps.strikes.get(id)?.total_pitches)??0,totalSwingingStrikes:number(hitterMaps.swing.get(id)?.total_pitches)??0,calledStrikes:number(hitterMaps.called.get(id)?.total_pitches)??0,foulStrikes:number(hitterMaps.foul.get(id)?.total_pitches)??0,threeOhPitches:number(hitterMaps.three.get(id)?.total_pitches)??0};});
...
const pitcherRows=pitcherData.base.map((r,i)=>{const id=Number(r.player_id);return{playerId:id,rank:i+1,player:r.player_name,totalPitches:number(r.total_pitches),totalSwingingStrikes:number(pitcherMaps.swing.get(id)?.total_pitches)??0,calledStrikes:number(pitcherMaps.called.get(id)?.total_pitches)??0,foulStrikes:number(pitcherMaps.foul.get(id)?.total_pitches)??0,threeOhPitches:number(pitcherMaps.three.get(id)?.total_pitches)??0,pullAirPct:number(pullMap.get(id)?.pull_air_rate)};});
