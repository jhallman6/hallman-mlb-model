...
export async function GET(request:NextRequest){
  ...
  try{
    ...
    const games=feeds.map((game:any)=>{const ap=pitcherFor(game,"away"),hp=pitcherFor(game,"home"),awayId=game.gameData?.teams?.away?.id,homeId=game.gameData?.teams?.home?.id,confirmedAway=lineupFor(game.liveData?.boxscore?.teams?.away,game.gameData),confirmedHome=lineupFor(game.liveData?.boxscore?.teams?.home,game.gameData),fgAway=datedFanGraphsLineup(date,game,"away"),fgHome=datedFanGraphsLineup(date,game,"home"),awayLineup=confirmedAway.length>=9?confirmedAway:fgAway?.players||projections.get(awayId)||[],homeLineup=confirmedHome.length>=9?confirmedHome:fgHome?.players||projections.get(homeId)||[],firstPitch=new Date(game.gameData?.datetime?.dateTime),started=game.gameData?.status?.abstractGameState!=="Preview";return{gamePk:game.gamePk,startTime:Number.isNaN(firstPitch.getTime())?null:firstPitch.toISOString(),gameTime:Number.isNaN(firstPitch.getTime())?"TBD":firstPitch.toLocaleTimeString("en-US",{timeZone:"America/New_York",hour:"numeric",minute:"2-digit"})+" ET",venue:game.gameData?.venue?.name||"Venue TBD",status:game.gameData?.status?.detailedState||"Scheduled",started,away:{name:game.gameData?.teams?.away?.name||"Away",abbreviation:game.gameData?.teams?.away?.abbreviation||"AWY",pitcher:ap.name,pitcherId:ap.id,throws:ap.throws,lineup:awayLineup,lineupStatus:confirmedAway.length>=9?"Confirmed lineup":fgAway?.status||awayLineup.length?"Projected lineup":"Projection unavailable"},home:{name:game.gameData?.teams?.home?.name||"Home",abbreviation:game.gameData?.teams?.home?.abbreviation||"HME",pitcher:hp.name,pitcherId:hp.id,throws:hp.throws,lineup:homeLineup,lineupStatus:confirmedHome.length>=9?"Confirmed lineup":fgHome?.status||homeLineup.length?"Projected lineup":"Projection unavailable"}}});
