...
function lineupFor(side:any,gameData:any){const players=side?.players||{};return(side?.battingOrder||[]).slice(0,9).map((id:number,index:number)=>{const player=players[`ID${id}`]||{},details=gameData?.players?.[`ID${id}`]||{};return{order:index+1,name:player.person?.fullName||details.fullName||"Unknown",bats:details.batSide?.code||"",position:player.position?.abbreviation||""}})}
function pitcherFor(game:any,side:"away"|"home"){const pitcher=game.gameData?.probablePitchers?.[side],details=pitcher?.id?game.gameData?.players?.[`ID${pitcher.id}`]:null;return{name:pitcher?.fullName||"TBD",throws:details?.pitchHand?.code||""}}
...
export async function GET(request:NextRequest){
  ...
  try{
    ...
    const games=feeds.map((game:any)=>{const ap=pitcherFor(game,"away"),hp=pitcherFor(game,"home"),awayId=game.gameData?.teams?.away?.id,homeId=game.gameData?.teams?.home?.id,confirmedAway=lineupFor(game.liveData?.boxscore?.teams?.away,game.gameData),confirmedHome=lineupFor(game.liveData?.boxscore?.teams?.home,game.gameData),awayLineup=confirmedAway.length>=9?confirmedAway:projections.get(awayId)||[],homeLineup=confirmedHome.length>=9?confirmedHome:projections.get(homeId)||[],firstPitch=new Date(game.gameData?.datetime?.dateTime);return{gamePk:game.gamePk,gameTime:Number.isNaN(firstPitch.getTime())?"TBD":firstPitch.toLocaleTimeString("en-US",{timeZone:"America/New_York",hour:"numeric",minute:"2-digit"})+" ET",venue:game.gameData?.venue?.name||"Venue TBD",status:game.gameData?.status?.detailedState||"Scheduled",away:{name:game.gameData?.teams?.away?.name||"Away",abbreviation:game.gameData?.teams?.away?.abbreviation||"AWY",pitcher:ap.name,throws:ap.throws,lineup:awayLineup,lineupStatus:confirmedAway.length>=9?"Confirmed lineup":awayLineup.length?"Projected lineup":"Projection unavailable"},home:{name:game.gameData?.teams?.home?.name||"Home",abbreviation:game.gameData?.teams?.home?.abbreviation||"HME",pitcher:hp.name,throws:hp.throws,lineup:homeLineup,lineupStatus:confirmedHome.length>=9?"Confirmed lineup":homeLineup.length?"Projected lineup":"Projection unavailable"}}});
    return NextResponse.json({date,fetchedAt:new Date().toISOString(),sourceUrl,fanGraphsStatus,source:fanGraphsStatus==="available"?"FanGraphs check + MLB live feed":"MLB live feed fallback",warning:"Confirmed lineups replace projections automatically. Before confirmation, each team uses its most recent starting batting order as the projection.",games},{headers:{"Cache-Control":"public, max-age=60, stale-while-revalidate=300"}})
