...
type GameLogSplit={date?:string;opponent?:{name?:string};isHome?:boolean;stat?:{gamesStarted?:number;pitchesThrown?:number}};
...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  const starts=(body.stats?.[0]?.splits||[]).filter(split=>(split.stat?.gamesStarted||0)>0&&Number.isFinite(Number(split.stat?.pitchesThrown))).sort((a,b)=>String(b.date||"").localeCompare(String(a.date||""))).slice(0,10).map(split=>({date:split.date||"",opponent:split.opponent?.name||"",location:split.isHome?"vs":"at",pitches:Number(split.stat?.pitchesThrown)}));
