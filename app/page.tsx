...
type Game = { gamePk:number; startTime?:string; started?:boolean; gameTime:string; venue:string; status:string; away:Team; home:Team };
...
function GameModel({data,loading,onRefresh}:{data:Payload|null;loading:boolean;onRefresh:()=>void}){
  const[selectedGame,setSelectedGame]=useState<number|null>(null),selected=data?.games.find(game=>game.gamePk===selectedGame);
  useEffect(()=>{if(selectedGame&&!selected)setSelectedGame(null)},[data,selected,selectedGame]);
  if(selected)return <MatchupLineupScreen game={selected} onBack={()=>setSelectedGame(null)}/>;
  return <><section className="model-picker-hero"><div><p className="eyebrow">Game model workspace</p><h1>Select a matchup,<br/><em>see the complete model.</em></h1><p>Lineups, hitter projections, pitcher projections, props, and F5 odds stay together here.</p></div><div className="model-picker-card"><label htmlFor="model-matchup">Matchup</label><select id="model-matchup" value={selectedGame??""} onChange={e=>setSelectedGame(e.target.value?Number(e.target.value):null)} disabled={loading}><option value="">{loading?"Loading matchups…":"Choose a matchup"}</option>{data?.games.map(game=><option key={game.gamePk} value={game.gamePk}>{game.away.abbreviation} at {game.home.abbreviation} · {game.gameTime}</option>)}</select><button onClick={onRefresh} disabled={loading}>{loading?"Refreshing…":"Refresh matchup data"}</button><small>{data?.games.length??0} games available on this slate</small></div></section>{!loading&&!data?.games.length&&<div className="no-games"><strong>No MLB games found.</strong><span>Return to Daily Lineups and choose another date.</span></div>}</>;
}
...
export default function Home() {
  ...
  return <main><header className="topbar"><div className="brand-mark">H</div><div><strong>The Hallman Algo</strong><span>MLB Game Model</span></div><nav><button className={view==="lineups"?"active":""} onClick={()=>setView("lineups")}>Daily Lineups</button><button className={view==="model"?"active":""} onClick={()=>setView("model")}>Game Model</button><button className={view==="bestBets"?"active":""} onClick={()=>setView("bestBets")}>Best Bets</button><button className={view==="statcast"?"active":""} onClick={()=>setView("statcast")}>Hitter Statcast</button><button className={view==="pitchers"?"active":""} onClick={()=>setView("pitchers")}>Pitchers</button></nav><OddsRefreshButton games={data?.games||[]}/><a href="https://www.fangraphs.com/leaders/major-league?pos=all&type=24&qual=1" target="_blank" rel="noreferrer">Open FanGraphs ↗</a></header><div className="page-shell">
    ...
    {loading?<div className="loading-grid">{Array.from({length:4}).map((_,i)=><div className="loading-card" key={i}/>)}</div>:data?.games.length?<div className="games-grid">{data.games.map(game=><article className="game-card" key={game.gamePk}><div className="game-meta"><div><span>{game.status}</span><strong>{game.gameTime}</strong></div><div className="game-actions"><small>{game.venue}</small></div></div><div className="matchup"><Lineup team={game.away}/><div className="at">AT</div><Lineup team={game.home}/></div></article>)}</div>:!error&&<div className="no-games"><strong>No MLB games found.</strong><span>Choose another date to load a slate.</span></div>}
