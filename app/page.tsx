...
export default function Home() {
  ...
  const selected=data?.games.find(game=>game.gamePk===selectedGame);
  return <main><header className="topbar"><div className="brand-mark">H</div><div><strong>The Hallman Algo</strong><span>MLB Game Model</span></div><nav><button className={view==="lineups"?"active":""} onClick={()=>setView("lineups")}>Daily Lineups</button><button className={view==="statcast"?"active":""} onClick={()=>setView("statcast")}>Hitter Statcast</button><button className={view==="pitchers"?"active":""} onClick={()=>setView("pitchers")}>Pitchers</button></nav><a href="https://www.fangraphs.com/leaders/major-league?pos=all&type=24&qual=1" target="_blank" rel="noreferrer">Open FanGraphs ↗</a></header><div className="page-shell">
    {view==="statcast"?<HitterStatcast/>:view==="pitchers"?<PitcherData/>:selected?<MatchupLineupScreen game={selected} onBack={()=>setSelectedGame(null)}/>:<>
    ...
    {loading?<div className="loading-grid">{Array.from({length:4}).map((_,i)=><div className="loading-card" key={i}/>)}</div>:data?.games.length?<div className="games-grid">{data.games.map(game=><article className="game-card" key={game.gamePk}><div className="game-meta"><div><span>{game.status}</span><strong>{game.gameTime}</strong></div><div className="game-actions"><small>{game.venue}</small><button onClick={()=>setSelectedGame(game.gamePk)}>Open matchup</button></div></div><div className="matchup"><Lineup team={game.away}/><div className="at">AT</div><Lineup team={game.home}/></div></article>)}</div>:!error&&<div className="no-games"><strong>No MLB games found.</strong><span>Choose another date to load a slate.</span></div>}
