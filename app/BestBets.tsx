...
type Game={gamePk:number;away:Team;home:Team};
...
export default function BestBets({games}:{games:Game[]}){
 ...
 const ranked=useMemo(()=>Object.values(groups).flatMap(group=>group.bets.map(bet=>({...bet,matchup:group.matchup} as RankedBet))).filter(bet=>bet.edge>=.07).sort((a,b)=>b.edge-a.edge),[groups]);
 const expected=games.length*2,loaded=Object.keys(groups).length;
 return <section className="best-bets-page"><div className="best-bets-hero"><div><p className="eyebrow">Slate-wide value board</p><h1>Best Bets</h1><p>Every available pitcher prop with at least a 7% model edge, ranked from strongest to weakest.</p></div><div className="best-bets-unit"><label>Dollar unit size<input inputMode="decimal" value={unitInput} onChange={event=>setUnitInput(event.target.value)}/></label><small>{loaded}/{expected} pitchers evaluated</small></div></div><div className="best-bets-summary"><div><span>Qualified bets</span><strong>{ranked.length}</strong></div><div><span>Minimum edge</span><strong>7.00%</strong></div><div><span>Recommended exposure</span><strong>${ranked.reduce((sum,bet)=>sum+bet.amount,0).toFixed(2)}</strong></div></div>{loaded<expected&&<div className="best-bets-loading"><span>Calculating the full slate</span><strong>{Math.round(loaded/Math.max(1,expected)*100)}%</strong></div>}{loaded===expected&&!ranked.length?<div className="no-games"><strong>No bets currently clear 7%.</strong><span>The board will update as sportsbook prices change.</span></div>:<div className="best-bets-table"><table><thead><tr><th>Rank</th><th>Matchup</th><th>Pitcher</th><th>Bet</th><th>Best book</th><th>Odds</th><th>Model probability</th><th>Edge</th><th>Units</th><th>Recommended bet</th></tr></thead><tbody>{ranked.map((bet,index)=><tr key={`${bet.matchup}-${bet.id}`}><td><strong>#{index+1}</strong></td><td>{bet.matchup}</td><td className="player-name">{bet.pitcher}</td><td><strong>{bet.selection} {bet.line}</strong><small>{bet.market}{bet.alternate?" · alternate":""}</small></td><td>{bet.book}</td><td>{odds(bet.odds)}</td><td>{pct(bet.modelProbability)}</td><td className="positive-cell">{pct(bet.edge)}</td><td>{bet.units.toFixed(2)}</td><td><strong>${bet.amount.toFixed(2)}</strong></td></tr>)}</tbody></table></div>}<div className="best-bet-calculators" aria-hidden="true">{games.flatMap(game=>(["away","home"] as const).map(side=><Calculator key={`${game.gamePk}-${side}`} id={`${game.gamePk}-${side}`} game={game} side={side} unitSize={unitSize} onUpdate={update}/>))}</div></section>;
}
